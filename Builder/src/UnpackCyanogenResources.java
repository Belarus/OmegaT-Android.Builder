...
public class UnpackCyanogenResources {
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (App app : translationInfo.getApp()) {
            ...
            for (Cyanogen cy : app.getCyanogen()) {
                String version = "cyanogen-" + getCyanogenDirVersion(args[0], cy.getSrc());
                ...
                outDir.mkdirs();
                copyIfExist(new File(args[0], cy.getSrc() + "/values/strings.xml"), new File(outDir,
                        "strings_" + version + ".xml"));
                copyIfExist(new File(args[0], cy.getSrc() + "/values/arrays.xml"), new File(outDir, "arrays_"
                        + version + ".xml"));
                copyIfExist(new File(args[0], cy.getSrc() + "/values/plurals.xml"), new File(outDir,
                        "plurals_" + version + ".xml"));
            }
        }
    }
    ...
    protected static String getCyanogenDirVersion(String gitsDir, String cyanogenDir) throws Exception {
        int p = cyanogenDir.indexOf('/');
        File head = new File(gitsDir, cyanogenDir.substring(0, p + 1) + ".git/HEAD");
