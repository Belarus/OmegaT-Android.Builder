...
public class UnpackCyanogenResources {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Translation translationInfo = UnpackBinaryResources.readTranslationInfo();
        ...
        for (App app : translationInfo.getApp()) {
            ...
            for (Cyanogen cy : app.getCyanogen()) {
                ...
                System.out.println("From dir " + cy.getSrc() + " version=" + version + " into "
                        + outDir.getAbsolutePath());

                outDir.mkdirs();
                copyIfExist(new File(args[0], cy.getSrc() + "/values/strings.xml"), new File(outDir, "strings_"
                        + version + ".xml"));
                copyIfExist(new File(args[0], cy.getSrc() + "/values/arrays.xml"), new File(outDir, "arrays_" + version
                        + ".xml"));
                copyIfExist(new File(args[0], cy.getSrc() + "/values/plurals.xml"), new File(outDir, "plurals_"
                        + version + ".xml"));
            }
        }
    }
